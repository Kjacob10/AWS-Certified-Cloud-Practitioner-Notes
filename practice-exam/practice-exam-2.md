...
1. A global company with a large number of AWS accounts is seeking a way in which they can centrally manage billing and security policies across all accounts. Which AWS Service will assist them in meeting these goals?
    - A. AWS Organizations.
    - B. AWS Trusted Advisor. [Provides best practice recommendations for cost, performance, and security]
    - C. IAM User Groups.  [manage permission for single account]
    - D. AWS Config. [Tracks resource configurations and compliance]
...
2. Which service provides object-level storage in AWS?
    - A. Amazon EBS. [block storage]
    - B. Amazon Instance Store. [ephemeral block storage physically attached to machine]
    - C. Amazon EFS. [file level storage]
    - D. Amazon S3. [object storage service]
...
3. A company is concerned that they are spending money on underutilized compute resources in AWS. Which AWS feature will help ensure that their applications are automatically adding/removing EC2 compute capacity to closely match the required demand?
    ...
    - B. AWS Budgets.
    - C. AWS Auto Scaling.
    - D. AWS Cost Explorer. [Cost Explorer helps visualize and analyze spending]
...
4. Which S3 storage class is best for data with unpredictable access patterns?
    - A. Amazon S3 Intelligent-Tiering. [moves objects between frequent and infrequent access tiers based on usage patterns.]
    - B. Amazon S3 Glacier Flexible Retrieval.[retrieval times ranging from minutes to hours]
    - C. Amazon S3 Standard. [Standard is for frequently accessed data]
    - D. Amazon S3 Standard-Infrequent Access. [data accessed infrequently but requires rapid access when needed]
...
5. What is the AWS database service that allows you to upload data structured in key-value format?
    - A. Amazon DynamoDB.
    - B. Amazon Aurora. [realtional]
    - C. Amazon Redshift. [data warehousing]
    - D. Amazon RDS. [relational database]
...
6. Which of the following is NOT correct regarding Amazon EC2 On-demand instances?
    - A. You have to pay a start-up fee when launching a new instance for the first time. [no startup pay for compute capacity]
...
7. A company has moved to AWS recently. Which of the following AWS Services will help ensure that they have the proper security settings? (Choose TWO)
    - A. AWS Trusted Advisor.[Provides best practice recommendations for cost, performance, and security]
    - B. Amazon Inspector. [automated security assessment service]
    - C. Amazon SNS.
    - D. Amazon CloudWatch.
...
8. What is the AWS feature that provides an additional level of security above the default authentication mechanism of usernames and passwords?
    ...
    - C. AWS KMS. [manages encryption keys]
    - D. AWS MFA.
...
9. A company is introducing a new product to their customers, and is expecting a surge in traffic to their web application. As part of their Enterprise Support plan, which of the following provides the company with architectural and scaling guidance?
    - A. AWS Knowledge Center. [repository of FAQs and documentation]
    - B. AWS Health Dashboard.
    - C. Infrastructure Event Management. [provides proactive architectural and scaling guidance from AWS experts during significant events ]
    - D. AWS Support Concierge Service.[Concierge helps with account and billing issues not technical]
...
11. Which of the below is a best-practice when designing solutions on AWS?
    ...
    - D. Provision a large compute capacity to handle any spikes in load [use asg and elb]
...
12. According to the AWS Acceptable Use Policy, which of the following statements is true regarding penetration testing of EC2 instances?
    - A. Penetration testing is not allowed in AWS. [allows with conditions]
    - B. Penetration testing is performed automatically by AWS to determine vulnerabilities in your AWS infrastructure.
    - C. Penetration testing can be performed by the customer on their own instances without prior authorization from AWS.[ as long as they follow AWS guidelines and test only permitted services.]
    - D. The AWS customers are only allowed to perform penetration testing on services managed by AWS. [Customers cannot penetration test AWS-managed services (like RDS or DynamoDB)]
...
13. Which service is used to ensure that messages between software components are not lost if one or more components fail?
    - A. Amazon SQS. [guarantees delivery of messages, stores them redundantly, and ensures they are not lost even if a component fails.]
    - B. Amazon SES. [seding and recieveing emails not reliable between components]
    - C. AWS Direct Connect. [dedicated network connection between on-premises and AWS]
    - D. Amazon Connect. [Connect is a cloud-based contact center service]
...
14. The principle “design for failure and nothing will fail” is very important when designing your AWS Cloud architecture. Which of the following would help adhere to this principle? (Choose TWO)
    ...
    - B. Availability Zones. [ensures high availability and fault tolerance]
    - C. Elastic Load Balancing. [distributes traffic across multiple instances and AZs]
    - D. Penetration testing.
    - E. Vertical Scaling. [incresae instance capacity but create single point of failure instead horizontal with redundacny]
...
15. What is the AWS service that provides a virtual network dedicated to your AWS account?
    - A. AWS VPN. [VPN connects your on-premises network to AWS securely,]
    - B. AWS Subnets.
    - C. AWS Dedicated Hosts.[Dedicated Hosts provide physical servers for compliance or licensing needs]
    - D. Amazon VPC. [VPC lets you provision a logically isolated section of the AWS Cloud]
...
18. Your company is designing a new application that will store and retrieve photos and videos. Which of the following services should you recommend as the underlying storage mechanism?
    ...
    - B. Amazon SQS.
    - C. Amazon S3.
    - D. Amazon Instance store. [ephermeral lost when instance stoped]
...
19. Which of the following is equivalent to a user name and password and is used to authenticate your programmatic access to AWS services and APIs?
    - A. Instance Password.
    - B. Key pairs.
    - C. Access Keys. [Access Key ID and a Secret Access Key]
    - D. MFA.
...
22. Which of the following EC2 instance purchasing options supports the Bring Your Own License (BYOL) model for almost every BYOL scenario?
    ...
    - B. Dedicated Hosts. [BYOL] [Dedicated Hosts give you full visibility and control over the physical server that your EC2 instances run on.]
...
23. Which of the following is one of the benefits of moving infrastructure from an on-premises data center to AWS?
    ...
    - C. Reduced Capital Expenditure (CapEx). [Aws Cloud =Opex]
...
24. Which of the following are important design principles you should adopt when designing systems on AWS? (Choose TWO)
    ...
    - B. Always choose to pay as you go. [Many AWS services are regional by design to ensure compliance, latency, and redundancy]
    - C. Treat servers as fixed resources. [should design for scaling up/down and automation, not treat them as fixed as they are elastic resource]
...
27. Which of the following services can help protect your web applications from SQL injection and other vulnerabilities in your application code?
    - A. Amazon Cognito. [handles user authentication and identity management,]
    - B. AWS IAM.
    - C. Amazon Aurora.
    - D. AWS WAF.
...
28. An organization needs to analyze and process a large number of data sets. Which AWS service should they use?
    - A. Amazon EMR. [big data platform that allows organizations to process and analyze massive datasets using frameworks like Apache Hadoop, Spark, Hive, and Presto]
    - B. Amazon MQ. [message broker service]
    - C. Amazon SNS.[pub/sub mssg model]
    - D. Amazon SQS. [message queuing service for decoupling applications,]
...
32. In your on-premises environment, you can create as many virtual servers as you need from a single template. What can you use to perform the same in AWS?
    - A. IAM.
    - B. An internet gateway. [Internet gateways provide internet connectivity for VPCs]
    - C. EBS Snapshot. [capture the state of block storage volumes, useful for backups and replication, ]
    - D. AMI. [template for ec2 , reusable server template (OS + apps + configs)]
...
35. Which statement best describes the operational excellence pillar of the AWS Well-Architected Framework?
    ...
    - D. The ability to manage datacenter operations more efficiently. [AWS manages the datacenter]
...
36. AWS has created a large number of Edge Locations as part of its Global Infrastructure. Which of the following is NOT a benefit of using Edge Locations?
    ...
    - C. Edge locations are used by CloudFront to distribute traffic across multiple instances to reduce latency. [ELB distribute traffic]
...
37. What are the change management tools that helps AWS customers audit and monitor all resource changes in their AWS environment? (Choose TWO)
    - A. AWS CloudTrail.
    - B. Amazon Comprehend. [NLP Service]
    - C. AWS Transit Gateway. [connects VPCs and on-premises networks]
    - D. AWS X-Ray. [helps analyze and debug distributed applications, but it’s focused on tracing requests]
    - E. AWS Config.
...
38. Which of the following services allows you to run containerized applications on a cluster of EC2 instances?
    - A. Amazon ECS. [lets you run Docker containers on a cluster of ec2 instance or fargate]
    - B. AWS Data Pipeline. [Data Pipeline is for data movement and transformation, ]
    - C. AWS Cloud9.[cloud based ide]
    - D. AWS Personal Health Dashboard. [alerts and remediation]
...
39. Which of the following services will help businesses ensure compliance in AWS?
    - A. CloudFront. [content delivery]
    - B. CloudEndure Migration. [migration service for moving workloads to AWS]
    - C. CloudWatch.
    - D. CloudTrail.[record API calls]
...
40. Which of the following procedures will help reduce your Amazon S3 costs?
    - A. Use the Import/Export feature to move old files automatically to Amazon Glacier. [imp/exp is for physically transferring data into AWS using devices]
    - B. Use the right combination of storage classes based on different use cases.
    - C. Pick the right Availability Zone for your S3 bucket.[s3 is regional service not tied to single az]
    - D. Move all the data stored in S3 standard to EBS.[ebs is block storage costs would increase]
...
42. Which of the following activities may help reduce your AWS monthly costs?
    ...
    - B. Using the AWS Network Load Balancer (NLB) to load balance the incoming HTTP requests. [nlb improves network performance]
    - C. Removing all of your Cost Allocation Tags. [help track and manage costs removing them would make it harder]
    - D. Deploying your AWS resources across multiple Availability Zones.[improve fault tolerant and availability]
...
44. Which of the following AWS security features is associated with an EC2 instance and functions to filter incoming traffic requests?
    - A. AWS X-Ray. [tracing and debugging applications]
    - B. Network ACL. [ACL is applied at subnet level not instance level]
    - C. Security Groups.
    - D. VPC Flow logs.
...
45. Which AWS services can be used to improve the performance of a global application and reduce latency for its users? (Choose TWO)
    - A. AWS KMS.
    - B. AWS Global accelerator. [uses the AWS global network and edge locations to route traffic optimally ]
...
46. Using Amazon RDS falls under the shared responsibility model. Which of the following are customer responsibilities? (Choose TWO)
    ...
    - B. Performing backups. [automate backups]
    - C. Managing the database settings.
    - D. Patching the database software. [aws manages patching of db ]
    - E. Installing the database software.[ aws installs and manages db software for rds]
...
47. A company has a large amount of structured data stored in their on-premises data center. They are planning to migrate all the data to AWS, what is the most appropriate AWS database option?
    - A. Amazon DynamoDB. [for unstructured]
    - B. Amazon SNS.
    - C. Amazon RDS.
...
48. A company has created a solution that helps AWS customers improve their architectures on AWS. Which AWS program may support this company? [check needed]
    - A. APN Consulting Partners.
    - B. AWS TAM.[provide guidance and best practices to enterprise customers]
    - C. APN Technology Partners.
    - D. AWS Professional Services. [aws own consulting arm not for external]
...
49. What is the AWS serverless service that allows you to run your applications without any administrative burden?
    - A. Amazon LightSail.
    - B. AWS Lambda.
    - C. Amazon RDS instances.[managed but no serverless still choose instance types]
...
50. Jessica is managing an e-commerce web application in AWS. The application is hosted on six EC2 instances. One day, three of the instances crashed; but none of her customers were affected. What has Jessica done correctly in this scenario?
    ...
    - D. She has properly built a scalable system. [refers to handling growth in demand, not resilience]
